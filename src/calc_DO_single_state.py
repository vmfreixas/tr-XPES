...
#       Cation state
    ...
    else: # The neutral molecule state is in the GS
        bAO = np.zeros((Nbf), dtype = float)
        for root in [state - 1]:
            print('Calculating DO for root ' + str(root + 1))
            # CI vector for the Cation molecule
            ...
            for kb in range(len(ciC)):
                for q in range(Nocc):
                    ...
                    MO_swapped[[k, b], :] = MO_swapped[[b, k], :]
                    MO_swapped = np.delete(MO_swapped[:Nocc, :Nocc], q, axis = 1)
                    bMO[q] += ciC[kb][2] * (-1) ** (Nbf + q) * (np.linalg.det(MO_swapped[:Nocc - 1, :Nocc - 1]))
...
neutralFile = '../examples/HBQ/tr_001/Neutral/step_01_0/tddft.out'
cationFile = '../examples/HBQ/tr_001/Cation_O/step_01_0/tddft.out'
Nbf = 519 
Nocc = 51
iNeutral = 1
state = 1
